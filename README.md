...
Este repositorio tiene como objetivo brindar **asistencia y ejemplos prácticos** a implementadores y desarrolladores que trabajan con **CALIPSO WAN**, proporcionando recursos de referencia, scripts y documentación técnica.

## Propósito

CALIPSO WAN es una plataforma empresarial desarrollada en Visual Basic 6 que utiliza motores de scripting (VBScript/JScript) para permitir la extensión dinámica del comportamiento de las aplicaciones.
...
- **`eventos_samples.vbs`**: Ejemplos de eventos DAF que incluyen:
  - **`daf_onbeforeexecute2`**: Evento que permite alterar propiedades de las columnas del DAF dinámicamente, controlando la visibilidad de campos según condiciones específicas (cotización, saldos, etc.).
  - **`daf_onbeforeview`**: Evento "pre-vista" que permite modificar completamente la estructura de una vista SQL antes de su ejecución, agregando tablas, joins, columnas calculadas, condiciones y ordenamiento.
...
### 📁 `/samples` - Ejemplos de uso (Appserver/API)

Ejemplos adicionales y “recetas” prácticas para escenarios comunes:

- **`login_methods.md`**: Ejemplos de ejecución de métodos/procedimientos por URL (endpoints de Appserver y API), incluyendo invocaciones con parámetros de login y consideraciones de seguridad.

## Uso de los Ejemplos

Los scripts de ejemplo están escritos en **VBScript** y están diseñados para ser integrados en los eventos de formularios y componentes DAF dentro de CALIPSO WAN. Cada ejemplo incluye comentarios y puede adaptarse a necesidades específicas.
